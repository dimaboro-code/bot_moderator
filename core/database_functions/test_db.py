...
"""
from core.database_functions.db_functions import add_user, in_database, add_lives, add_mute, get_user, \
    get_last_mute, delete_lives, db_unmute, delete_all_lives, add_or_update_id, \
    check_known_id, get_id, delete_old_data, delete_row, engine as eng
...
async def test_simple_db(engine: AsyncEngine = CONN) -> bool:
    ...
    try:
        ...
        await in_database(user_id=test_user_id)
        ...
        await add_or_update_id(user_id=test_user_id, username='dds')
        await check_known_id(user_id=test_user_id)
        await get_id(username='dds')
        await delete_old_data(days=0, user_id=test_user_id)
        await delete_row(user_id=test_user_id)
